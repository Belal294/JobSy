...
import HeroBackGround from "../../assets/BackGroundsImages/HeroBackGround.jpg"; // Ensure this path is correct
import UseFetchJobList from "../Hooks/UseFetchJobList";
 // Import the UseFetchJobList hook

export default function Hero() {
  const navigate = useNavigate(); // Initialize navigate hook

  // Use the UseFetchJobList hook to get all job data
  const fetchedJobs = UseFetchJobList();
  ...
  // Derive unique locations and job types from fetchedJobs using useMemo
  // This ensures these lists are re-calculated only when fetchedJobs changes
  const uniqueLocations = useMemo(() => {
    ...
    // Add an empty option for "Select Region" and sort alphabetically
    return ['', ...Array.from(locations).sort()];
  }, [fetchedJobs]); // Recalculate when fetchedJobs changes
  ...
  const uniqueJobTypes = useMemo(() => {
    ...
    // Add an empty option for "Select Job Type" and sort alphabetically
    return ['', ...Array.from(types).sort()];
  }, [fetchedJobs]); // Recalculate when fetchedJobs changes

  // Determine loading state based on whether fetchedJobs has data
  // Assuming UseFetchJobList handles its own internal loading/error and returns an empty array initially
  const isLoadingOptions = fetchedJobs.length === 0 && uniqueLocations.length === 0 && uniqueJobTypes.length === 0;
  ...
  const handleSearch = () => {
    ...
    if (selectedRegion) {
      queryParams.append("location", selectedRegion);
    }
    if (selectedJobType) {
      queryParams.append("job_type", selectedJobType);
    }

    // Navigate to the job list page with query parameters
    // Using '/jobs' as the path for job listings, as discussed previously
            ...
            <div className="bg-opacity-10 backdrop-blur-sm rounded-lg p-6 max-w-4xl mx-auto w-full">
              {isLoadingOptions ? (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
                  ...
                  >
                    <option value="" className="text-gray-800">Select Region</option>
              ...
              <div className="flex flex-wrap items-center mt-6 gap-2">
                ...
                {["UI Designer", "Python", "Developer"].map((kw) => (
                  <div
                    ...
                    onClick={() => setJobTitle(kw)} // Set job title when clicking a trending keyword
                  >
                    {kw}
                  </div>
                ))}
